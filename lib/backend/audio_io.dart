...
Future<ProcessedAudio> loadAudio(String filePath, int? sampleRate) async {
  ...
  final targetRate = sampleRate ?? wav.samplesPerSecond;
  if (wav.samplesPerSecond != targetRate) {
    samples = _resampleLinear(samples, wav.samplesPerSecond, targetRate);
  }

  return ProcessedAudio(samples, targetRate);
}
